...
function VideoCapture() {
  ...
  const { setPending } = useContext(ProfileContext);
  const pInit = useRef(
    null as Promise<{
  ...
  const init = async (): Promise<{
    ...
    try {
      ...
      resultReceiver.onDecodedBarcodesReceived = (
        ...
        console.log(result?.barcodeResultItems[0]?.text);
        setPending(false);

        localStorage.setItem('barcode', result?.barcodeResultItems[0]?.text);
      };
  ...
  return (
    <div>
      <div ref={uiContainer} className="w-[500px] h-[500px]"></div>
